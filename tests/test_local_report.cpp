...
void test_report_mode_requires_connection_path() {
	qase_reporter_reset();
	qase_reporter_add_result("DummyTest", true);

	QaseConfig cfg;
	cfg.mode = "report";
	cfg.report_connection_path = "";  // deliberately invalid
	...
	try {
		qase_submit_report(api, http, cfg);
		assert(false && "Expected exception due to missing report_connection_path");
	} catch (const std::invalid_argument& e) {
		assert(std::string(e.what()).find("report_connection_path") != std::string::npos);
	}
}
